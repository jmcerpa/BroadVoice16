                ...
                Word16 *e)   /* output: enhanced speech signal          */
{
   ...
   *ma_a = round(a0);
   ...
   for(n=0; n<NINT; n++)
   {
      ...
      bi0 = round(L_mac(L_mult(ww1, bb[0]), ww2, b_prv[0]));
      bi1c= mult(ww1, bb[1]);
      bi1p= mult(ww2, b_prv[1]);
      e[n] = round(L_mac(L_mac(L_mult(bi1c, s[XQOFF+n-ppnew]), bi1p, s[XQOFF+n-(*pp_prv)]), bi0, s[XQOFF+n]));
   }
   for(n=NINT; n<FRSZ; n++)
   {
      e[n] = round(L_shl(L_mult(gainn, e[n]),1));
   }
